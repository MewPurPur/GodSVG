...
func _ready() -> void:
	...
	SVG.xnodes_moved_in_parenet.connect(_on_xnodes_moved_in_parenet)
...
func _on_xnodes_moved_in_parenet(parent_xid: PackedInt32Array, indices: Array[int]) -> void:
	var old_selected_xids := selected_xids.duplicate()
...
func merge_selected() -> void:
	SVG.root_element.merge_xnodes(selected_xids)
	SVG.queue_save()
...
func get_selection_context(popup_method: Callable, context: Context) -> ContextPopup:
	...
	if not selected_xids.is_empty():
		...
		for base_xid in filtered_xids:
			if not XIDUtils.are_parents_same(base_xid, filtered_xids[0]):
				can_move_down = false
				can_move_up = false
				break
