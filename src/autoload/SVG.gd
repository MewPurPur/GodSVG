...
func _ready() -> void:
	...
	xnodes_layout_changed.connect(queue_update)
...
func sync_elements() -> void:
	...
	if svg_parse_result.error == SVGParser.ParseError.OK:
		...
		root_element.xnodes_moved_in_parenet.connect(xnodes_moved_in_parenet.emit)
		root_element.xnodes_moved_to.connect(xnodes_moved_to.emit)
		root_element.xnodes_layout_changed.connect(xnodes_layout_changed.emit)
