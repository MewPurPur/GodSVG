...
func render_handle_textures() -> void:
	...
	for i in Handle.Display.size():
		for ii in Utils.InteractionType.size():
			...
			if not atlas_textures.has(i):
				atlas_textures[i] = {}
			atlas_textures[i][ii] = atlas_texture
	queue_redraw()
...
func _draw() -> void:
	...
	draw_objects_of_interaction_type(normal_color, normal_polylines, normal_multiline, normal_handles, 0)
	draw_objects_of_interaction_type(hovered_color, hovered_polylines, hovered_multiline, hovered_handles, 1)
	draw_objects_of_interaction_type(selected_color, selected_polylines, selected_multiline, selected_handles, 2)
	draw_objects_of_interaction_type(hovered_selected_color, hovered_selected_polylines, hovered_selected_multiline, hovered_selected_handles, 3)
...
handles_array: Array[Handle], interaction_type: int) -> void:
	...
	for handle in handles_array:
		atlas_textures[handle.display_mode][interaction_type].draw(surface,
