...
func set_pos(new_pos: PackedFloat64Array) -> void:
	if precise_pos != new_pos:
		...
		if Input.is_action_pressed(&"control_point_axis_lock"):
...
class LineConstraint:
	...
	static func constrain(
		...
		# care about on the other end (opp for opposite). Calculate the slope, and use it to transform
		# the input.
		...
		var slope = opp[0] / opp[1]
		if abs(slope) > 1.0:
			pos = PackedFloat64Array([pos[0], pos[0] / slope])
		else:
			pos = PackedFloat64Array([pos[1] * slope, pos[1]])
