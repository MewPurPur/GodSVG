...
# Might not return a real XID.
static func unsafe_get_next_xid(xid: PackedInt32Array) -> PackedInt32Array:
	var next_xid := xid.duplicate()
	next_xid[-1] += 1
	return next_xid

static func are_parents_same(xid1: PackedInt32Array, xid2: PackedInt32Array) -> bool:
