...
func inject_ui_to_control(main_container: VBoxContainer, dimensions: Vector2) -> void:
	...
	main_container.add_child(quality_hbox)
	...
	background_edit.setup(false, PackedStringArray(["#ffffff", "#000000"]), background_color)
	background_hbox.add_child(background_edit)
	var export_scale_config := ExportScaleConfigScene.instantiate()
	export_scale_config.setup(dimensions, upscale_amount, 65535)
	...
	var _on_changed :=\
		func() -> void:
			...
			background_edit.set_color_no_signal(background_color.to_html(false))
	...
	HandlerGUI.register_focus_sequence(main_container, [quality_edit, background_edit, export_scale_config])
