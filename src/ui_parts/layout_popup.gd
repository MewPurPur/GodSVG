...
func _can_drop_data(at_position: Vector2, data: Variant) -> bool:
	...
	for section in section_areas:
		...
		if at_position.y < lerpf(section_area.position.y, section_area.end.y, 0.2) and\
		((section == SaveData.LayoutLocation.TOP_LEFT and\
		(bottom_left_parts.is_empty() or (bottom_left_parts.size() == 1 and\
		bottom_left_parts[0] == dragged_data.layout_part))) or\
		(section == SaveData.LayoutLocation.BOTTOM_LEFT and top_left_parts.is_empty() and\
		Configs.savedata.get_layout_parts(dragged_data.layout_location).size() >= 2)):
			...
			# Case 2: The section is bottom left, but the dragged layout part is not the only
			# one on the bottom left and there's nothing on the top left.
			proposed_drop_location_direction = DropDirection.ABOVE
			proposed_drop_idx = -1
		elif at_position.y > lerpf(section_area.position.y, section_area.end.y, 0.8) and\
		((section == SaveData.LayoutLocation.BOTTOM_LEFT and\
		(top_left_parts.is_empty() or (top_left_parts.size() == 1 and\
		top_left_parts[0] == dragged_data.layout_part))) or\
		...
		Configs.savedata.get_layout_parts(dragged_data.layout_location).size() >= 2)):
			...
			if proposed_drop_location_pivot == SaveData.LayoutLocation.EXCLUDED:
				...
				proposed_drop_idx = clampi(roundi(((at_position.x -\
						section_areas[section].get_center().x) / PART_UI_SIZE) * 2.0 +\
						layout_parts_count / 2.0), 0, layout_parts_count)
		
		if proposed_drop_location_direction == DropDirection.INSIDE and\
		proposed_drop_location_pivot == dragged_data.layout_location and\
