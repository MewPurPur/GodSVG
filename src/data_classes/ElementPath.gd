...
func user_setup(precise_position := PackedFloat64Array([0.0, 0.0])) -> void:
	if precise_position != PackedFloat64Array([0.0, 0.0]):
		...
		attrib.set_command_property(0, "x", precise_position[0])
		attrib.set_command_property(0, "y", precise_position[1])
...
func get_bounding_box() -> Rect2:
	...
	for cmd_idx in pathdata.get_command_count():
		...
		match cmd_char:
			...
			"C", "S":
				# Cubic Bezier curve
				var cp1 := Utils64Bit.get_vector(pathdata.get_implied_S_control(cmd_idx)) if cmd_char == "S" else Vector2(cmd.x1, cmd.y1)
				...
				var i := cp1 - Vector2(cmd.start_x, cmd.start_y)
				var j := Vector2(cmd.x2, cmd.y2) - cp1
			...
			"Q", "T":
				...
				var cp1 := pathdata.get_implied_T_control(cmd_idx) if cmd_char == "T" else PackedFloat64Array([cmd.x1, cmd.y1])
