...
signal xnodes_moved_in_parenet(parent_xid: PackedInt32Array, old_indices: Array[int])
signal xnodes_moved_to(xids: Array[PackedInt32Array], location: PackedInt32Array)
signal xnodes_layout_changed  # Emitted together with any of the above 4.
...
func move_xnodes_in_parent(xids: Array[PackedInt32Array], down: bool) -> void:
	...
	if old_indices != range(old_indices.size()):
		xnodes_moved_in_parenet.emit(parent_xid, old_indices)
		xnodes_layout_changed.emit()
...
func move_xnodes_to(xids: Array[PackedInt32Array], location: PackedInt32Array) -> void:
	...
	for id in xids_stored:
		if not XIDUtils.are_parents_same(id, location) or id[-1] < location[-1] or\
...
func merge_xnodes(xids: Array[PackedInt32Array]) -> void:
	get_xnode(xids[0]).merge()
	xnodes_layout_changed.emit()
